/* global $ */
import { PieceElement, WidgetInitOptions, WidgetPiecesElementData, WidgetPiecesElementParams } from "../../../types";
import ItemElement from "../item/item.element";
import CanvasHelper from "./canvas.helper";

export default abstract class PiecesElement extends ItemElement {
    ...
    async init(values?: any, options: WidgetInitOptions = { regenerateId: false }): Promise<void> {
        await super.init(values, options);
        const constructor = this.constructor as typeof PiecesElement;
        this.params = values?.params ? structuredClone(values.params) : {
            name: constructor.widget + "-" + this.id,
            help: ""
        };
        if (options.regenerateId) this.params.name = constructor.widget + "-" + this.id;
    ...
    protected addPiece(e: JQuery.TriggeredEvent<HTMLElement>) {
        ...
        import('./piece.hbs').then(({ default: piece }) =>
            $('#f-' + this.id).find('.pieces').append(piece(
                { ...rect, pos, widget: (this.constructor as typeof PiecesElement).widget })));
    }
    ...
    settingsOpened() {
        const self = this;
        const constructor = self.constructor as typeof PiecesElement;
        let $form = $('#f-' + self.id);
    ...
    validateModel(): string[] {
        ...
        if (this.data.pieces.length > 1) {
            this.data.pieces.forEach(piece => {
                this.utils.isStringEmptyOrWhitespace(<string>piece.altImg) && errors.push(constructor.widget + ".piece.altImg.invalid");
                this.utils.isStringEmptyOrWhitespace(<string>piece.altRec) && errors.push(constructor.widget + ".piece.altRec.invalid");
    ...
    validateForm(form: any): string[] {
        let errors: string[] = [];
        const constructor = this.constructor as typeof PiecesElement;
        this.utils.isStringEmptyOrWhitespace(form.alt) && errors.push("common.alt.invalid");
        ...
        if (form['piece'] && Array.isArray(form['piece'])) {
            form['piece'].forEach(piece => {
                this.utils.isStringEmptyOrWhitespace(piece.altImg) &&
                    errors.push(constructor.widget + ".piece.altImg.invalid");
                this.utils.isStringEmptyOrWhitespace(piece.altRec) &&
