...
export default class WidgetTrueFalseContainer extends WidgetElement {
    config = {
    ...
    preview(model) {
        ...
        element.querySelector("span").innerHTML = model.params.name ? model.params.name : this.translate("widgets.TrueFalseContainer.label");
        return element;
    }
    ...
    validateModel(widget) {
        var keys = [];
        if (widget.data.length == 0) keys.push(" TrueFalseContainer.data.empty");
