...
import { InputWidgetTableData, WidgetInitOptions, WidgetTableData, WidgetTableParams } from "../../../types";
import ItemElement from "../item/item.element";

export default class TableElement extends ItemElement {
    ...
    async init(values?: InputWidgetTableData, options: WidgetInitOptions = { regenerateId: false }): Promise<void> {
        await super.init(values, options);
        this.params = values?.params ? structuredClone(values.params) : {
            name: TableElement.widget + "-" + this.id,
            help: ''
        };
        if (options.regenerateId) this.params.name = TableElement.widget + "-"
    ...
    settingsOpened(): void {
        import("datatables.net-dt")
            ...
            .then(({ default: DataTable }) => {
                let $form = $('#f-' + this.id);
                const $table = $('#table');
                const deleteColumnStr = this.i18n.value("widgets.Table.form.deleteColumn");
                ...
                let initTable = (columns: string[], data: any[]) => {
                    ...
                    $table.DataTable({
                        ...
                        keys: true,
                        buttons: [
                            { text: this.i18n.value("widgets.Table.form.addRow"), action: addRow },
                            { text: this.i18n.value("widgets.Table.form.deleteRow"), action: deleteRow }]
                    });

                    $table.find('th').attr('tabindex', 0);
                    $table.DataTable().on('key', (e, datatable, key, cell, originalEvent) => {
                        if (key === 13) {
                            e.preventDefault();
