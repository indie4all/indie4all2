...
export default class ImageLocalElement extends ImageElement {
    ...
    async init(values?: InputWidgetImageData, options: WidgetInitOptions = { regenerateId: false }): Promise<void> {
        ...
        this.params = values?.params ? structuredClone(values.params) : {
            name: ImageLocalElement.widget + "-" + this.id,
            help: "",
        };
        if (options.regenerateId) this.params.name = ImageLocalElement.widget + "-" + this.id;
