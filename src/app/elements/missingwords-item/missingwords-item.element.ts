...
import { InputWidgetMissingwordsItemData, WidgetInitOptions, WidgetMissingwordsItemData } from "../../../types";
import SpecificItemElement from "../specific-item/specific-item.element";

export default class MissingWordsItemElement extends SpecificItemElement {
    ...
    async init(values?: InputWidgetMissingwordsItemData, options: WidgetInitOptions = { regenerateId: false }): Promise<void> {
    ...
    validateModel(): string[] {
        var errors: string[] = [];
        if (this.utils.isStringEmptyOrWhitespace(this.data.sentence))
            errors.push("MissingWords.sentence.empty");
        if (!MissingWordsItemElement.validateQuestionBlankSpots(this.data.sentence))
        ...
        this.data.combinations.forEach(combination => {
            this.utils.isStringEmptyOrWhitespace(combination) &&
    ...
    validateForm(form: any): string[] {
        var errors: string[] = [];
        if (this.utils.isStringEmptyOrWhitespace(form.sentence))
            errors.push("MissingWords.sentence.empty");
        if (!MissingWordsItemElement.validateQuestionBlankSpots(form.sentence))
            errors.push("MissingWords.sentence.onlyOneBlank");
