...
(function () {
  ...
  let onLanguageLoaded = function (err, i18n) {
    ...
    $('.pictograma').on('click', '.response', function (e) {
      e.preventDefault();
      const $widget = $(this).closest('.pictograma');
      const id = $widget.attr('id');
      var gamification = $('body').hasClass('gamification') && !$('body').hasClass('gamification-teacher');
      ...
      $(this).trigger('gamificate', [correcto == respuesta, correcto == respuesta ? 1 : 0, id]);
      if (!gamification) {
        ...
        createDragDrop: function (optionsContainerSelector, selectedOptionsContainerSelector, definitionsContainerSelector, ddTerms, objetivop, identificadorp) {
            ...
            let onLanguageLoaded = function (err, i18n) {
                ...
                let validate = function () {
                    ...
                    $('#check-' + widgetId).trigger('gamificate', [validCount == ddTerms.length, validCount / ddTerms.length, widgetId]);
                }
...
(function () {
	...
	let visualizandoVimeo = async function(objPlayer) {
		const time = await objPlayer.player.getCurrentTime();
		if (objPlayer.interval != 0) {
			if (Math.abs(time - objPlayer.interval) > 2.0) {
				objPlayer.range.push({ start: time, end: time });
				objPlayer.element = objPlayer.element + 1;
			} else objPlayer.range[objPlayer.element].end = time;
		} else {
			objPlayer.range.push({ start: time, end: time });
		}
		objPlayer.interval = time;
	}
	...
	let checkPauseVimeo = async function(objPlayer, timePause) {
		const currentTime = await objPlayer.player.getCurrentTime();
	...
	let onLanguageLoaded = function (err, i18n) {
		...
		let processVideo = function () {
			...
			if ($(this).hasClass('vimeo')) {
				...
				if ($(this).hasClass('open'))
					player.player.on('ended', () => { setObjetivoCompleto("objetivo" + id, "Video" + id + "/" + title, "Video" + id); });
				else if ($(this).hasClass('interoperability')) {
					player.player.on('ended', () => { sendInteractionVimeo(player); });
					player.player.on('pause', async () => {
						const time = await player.player.getCurrentTime();
						setTimeout(() => checkPauseVimeo(player, time), 1000)
					});
					player.player.on('timeupdate', function () { visualizandoVimeo(player); });
				}
			} else if ($(this).hasClass('youtube')) {
				let self = this;
				let partvideo = videoId.split('/embed/')[1];
				let part1 = partvideo.split('?')[0];
				let parameters = partvideo.split('?')[1].split('&');
				let start = parameters[0].split('=')[1];
				let options = {start};
				if (parameters.length > 1) {
					options["end"] = parameters[1].split('=')[1];
				}
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        let checkGamificationCouples = function (e) {
            ...
            $(this).trigger('gamificate', [correct, score, id]);
        }
        ...
        $('.couples').each(function () {
            ...
            if (!gamification) {
                ...
                // If the current user is a teacher, send the corresponding event to the gamification server
                if ($('body').hasClass('gamification-teacher'))
        ...
        // The terms may contain math expressions, so we need to typeset them
        if (MathJax && typeof MathJax.typesetPromise === 'function') {
            // The initial typesetting runs asycnronously, so we need to wait for it to finish
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.container-test').each(function () {
            ...
            const gamification = $('body').hasClass('gamification') && !$('body').hasClass('gamification-teacher');
            const $widget = $(this);
            const $startButton = $widget.find('.container-test-btn');
            const $container = $widget.closest('.content-element');
            const $elementsToHideWhenQuestion = $container.find('.container-test-start,.container-test-heading');
            const $questions = $widget.find('.container-test-question');
            const $questionProgress = $widget.find(".container-test-current-question")
            let self = this;
            let id = $(self).attr('id');
            let answers = {};
            let totalScore = 0;
            ...
            let onSpeechToAction = function (event, action, entities) {
                ...
                if (action === "FINISH") {
                    !$startButton.is(':visible') && event.preventDefault();
                    return;
                }
                
                $startButton.focus();
                if (action === "START" || action === "CURRENT" || action === "NEXT" || 
                    action === "PREVIOUS" || action === "GOTO") {
                    return;
                }
                else if (action === "CLOSE" && ($(".modal.show").length || $(".alert").length)) {
                    $(".modal.show").length && $('.modal.show').modal('hide');
                    ...
                    switch (action) {
                        case "SELECT": $startButton.trigger('click'); break;
                        case "REPEAT": $(self).find('.tta-message').append(makeAlert('alert-info', i18n('tta-info-containertest'))); break;
                        case "RESET": $(self).find('.btn-container-test-reset').trigger('click'); break; // OK
                        case "NONE":
                        default:
                            $(self).find('.tta-message').append(makeAlert('alert-danger', i18n('tta-error-message')));
                            break;
            ...
            $startButton.on('click', function() {
                ...
                $elementsToHideWhenQuestion.toggleClass('d-none', true);
                $questionProgress.toggleClass('show', true);
                $questionProgress.html(i18n("container-test-current-question", {current: 1, total: $questions.length}));
                $questions.first().toggleClass('show', true).focus();
                $container.find('.speech-to-action.active').last().trigger('speech-hidden');
            });

            // When the teacher sends the event, advance to the next question
            $widget.on('gamification-step', function (e, step) {
                $questions.removeClass("show");
                const $currentQuestion = $questions.eq(step);
                // There are no more questions
                if (!$currentQuestion.length) {
                    activityStarted = false;
                    $elementsToHideWhenQuestion.toggleClass('d-none', false);
                    $questionProgress.removeClass('show');
                    $widget.addClass('finished');
                    return;
                }
                activityStarted = true;
                $elementsToHideWhenQuestion.toggleClass('d-none', true);
                $questionProgress.html(i18n("container-test-current-question", {current: step+1, total: $questions.length}));
                $questionProgress.addClass('show');
                $currentQuestion.addClass("show").focus();
                $container.find('.speech-to-action.active').last().trigger('speech-hidden');
            });

            $widget.on('gamification-start-student', function (e) {
            ...
            $widget.on('gamificate', function (e, correct, score, id) {
                answers[id] = [correct, score];
                // Get the sum of score in the answers object
                totalCorrect = Object.values(answers).map(arr => arr[0]).reduce((acc, correct) => acc + (correct ? 1 : 0), 0);
                totalScore = Object.values(answers).map(arr => arr[1]).reduce((acc, score) => acc + score, 0);
                if (gamification) $(e.target).attr("disabled", true);
                // the teacher will show the next question (in gamification) by pressing next activity
                if (correct && !gamification && !$('body').hasClass('gamification-teacher'))  {
                    setTimeout(() => {
                        const $currentQuestion = $(e.target).closest(".container-test-question");
                        const $nextQuestion = $currentQuestion.next(".container-test-question");
                        $currentQuestion.removeClass("show");
                        $nextQuestion.addClass("show").focus();
                        if (!$nextQuestion.length) {
                            activityStarted = false;
                            $widget.addClass('finished');
                            $startButton.prop('disabled', true);
                            if (!gamification) $widget.find(".container-test-result").html(i18n("TextNumberCorrect", {num: totalCorrect}));
                            setObjetivoCompleto("objetivo" + $widget.attr('id'), $widget.data('desc'), $widget.data('desc'));
                            $elementsToHideWhenQuestion.toggleClass('d-none', false);
                            $questionProgress.removeClass('show', true);
                        } else {
                            $questionProgress.html(i18n("container-test-current-question", {current: $questions.index($nextQuestion)+1, total: $questions.length}));
                        }
                        $container.find('.speech-to-action.active').last().trigger('speech-hidden');
                    }, 3000);
                }
            ...
            $widget.on('click', '.btn-container-test-reset', function (e) {
                answers = {};
                activityStarted = false;
                $elementsToHideWhenQuestion.toggleClass('d-none', false);
                $startButton.prop('disabled', false);
                $widget.removeClass('finished');
                $widget.find(".container-test-question").toggleClass("show", false);
                $questionProgress.toggleClass("show", false);
                $widget.find(".container-test-result").empty();
                $widget.find(".btn-reset").trigger('click');
            ...
            if (gamification) {
                // The student cannot play until they receive a signal from the teacher
                $widget.addClass("finished");
                $startButton.prop('disabled', true);
            }
        });
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.boton_truefalse').click(function () {
            ...
            $(this).trigger('gamificate', [numCorrectAnswers == solutions.length, numCorrectAnswers / solutions.length, id]);
            if (!gamification) {
...
(function () {
    ...
    if (typeof annyang !== 'undefined' && annyang) {
        ...
        let onLanguageLoaded = function (err, i18n) {
            ...
            let preprocessCandidates = function (candidates) {
                ...
                return candidates
                    ...
                    .map(candidate => {
                        ...
                        let numbersArr = i18n('tta-numbers', { returnObjects: true });
                        return tokens.map(token => numbersArr.includes(token.toLowerCase()) ? numbersArr.indexOf(token.toLowerCase()) : token).join(' ')
                    })
            }
            ...
            // Roulette and container test hide and show the current question
            // this function transfers the control to the following question
            let onContextHidden =  function() {
                if (currentTarget !== this) return;
                // Find the speech-to-action button corresponding to the current question
                const $target = $(currentTarget).closest('.question-container').length ? 
                    $(currentTarget).closest('.question-container').closest('.content-element') : 
                    $(currentTarget).closest('.content-element');
                const candidateContext = $target.find('.speech-to-action:visible').first().get(0);
                // Notify the finish command to the current voice-controlled button
                let evt = sendCommand.apply(currentTarget, ['FINISH']);
                if (!evt.isDefaultPrevented()) {
                    switchVoiceButtonsState.apply(currentTarget, [false, false])
                    finishRecording.apply(currentTarget, [false]);
                }
                // If the previous context is the current one, there is no need to create it by clicking
                if (candidateContext && candidateContext !== currentTarget) {
                    $(candidateContext).trigger('click');
                }
            }
            ...
            $(document).on('speech-hidden', '.speech-to-action', onContextHidden);
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.widget-missing-words').each(function () {
            ...
            $(this).on('click', '.btn-check-answer', function () {
                ...
                $(this).trigger('gamificate', [score === total, score / total, id]);
                if (!gamification) {
...
(function () {
    ...
    let show = function() {
        ...
        $text.addClass('show');
        setTopLinkIfNeeded.apply($widget.get(0));
        // Mark the objetive as complete when the tab is shown
        setObjetivoCompleto('objetivo' + this.getAttribute("id"), this.dataset.desc, this.dataset.type);
    }
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.sentences-container').each(function () {
            ...
            $(this).on('click', '.btn-check-answer', function () {
                ...
                $(this).trigger('gamificate', [score === $widgets.length, score / $widgets.length, id]);
                if (!gamification) {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        let init = function () {
            ...
            $widget.closest('.content-element').on('click', '.btn-check-answer', function () {
                ...
                let gamification = $('body').hasClass('gamification') && !$('body').hasClass('gamification-teacher');
                let $terms = $(this).closest('.content-element').find('.column-term');
                ...
                let $wrongTerms = $terms.filter(function () {
                    ...
                    return idx === -1 || !solutions[idx].includes($(this).find('.term-button').data('term'));
                });
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.widget-puzzle').each(function () {
            ...
            let drag = function (e) {
                // Hide previously open dropdowns
                $widget.find(".dropdown-menu.show").dropdown('toggle');
            ...
            tmpImage.onload = function () {
                ...
                let onSpeechToAction = function (event, action, entities) {
                    ...
                    if ($widget.find('.dropdown-menu.show').length)
                        mode = NAVIGATION_OPTIONS;
                    else
                        mode = NAVIGATION_TERMS;
                    ...
                    if (mode === NAVIGATION_TERMS)
                        ...
                        let $dropdown = $widget.find('.dropdown-menu.show').first();
                ...
                $widget.closest('.content-element').on('click', '.btn-check-answer', function () {
                    ...
                    $(this).trigger('gamificate', [score === rects.length, score / rects.length, id]);
                    if (!gamification) {
...
(function() {
	...
	$(document).ready(function() {
		...
		fetch(i18nURL)
			.then((response) => response.json())
			.then((json) => {
				$('.widget-table').on('init.dt', function(e) {
					// Add the Bootstrap class table-responsive when the table has been initialized
					$(e.currentTarget).closest('.dataTables_wrapper').addClass("table-responsive");
				}).DataTable({
					ordering: true,
					paging: true,
					searching: true,
...
(async function () {
    ...
    // Get the vt query parameter, used in preview units
    const paramVT = params.get('vt');
    // vt query parameter from the referer unit, used in preview units
    let refererParamVT = '';
    let content;
    ...
    if (paramReferer) {
        try {
            ...
            const parsed = new URL(paramReferer);
            // URL to retrieve the referer table of contents
            const url = `${(parsed.origin + parsed.pathname).replace(/\/$/, "")}/related_units.json`;
            ...
            // Get the vt query parameter from the referer unit, used in preview units
            const refererParams = new URLSearchParams(parsed.search);
            if (refererParams.has('vt')) refererParamVT = refererParams.get('vt');
        } catch (e) {
            content = null;
        }
    }
    ...
    const parseRelatedUnit = (unit, level = 3) => {
        ...
        let current, url, originQueryParam = "", refererVTQueryParam = "";
        // # is a placeholder to mark the current unit of the loaded JSON file
        if (unit.url === '#') {
            url = referer;
            // Add the vt query parameter to the url if we come from a preview unit
            if (refererParamVT) refererVTQueryParam = "&vt=" + refererParamVT;
        } else {
            const parsed = new URL(unit.url);
            let hostname = parsed.hostname;
            // Only replace hostname if the target url is opencontent or contentserver
            if (ALLOWED_HOSTS.includes(hostname)) {
                // AND we are not in a preview, preview units won't rewrite the hostname
        ...
        // paramVT is the vt query parameter of the current unit 
        //  -> will be used by the REDIRECTED unit
        // refererVTQueryParam is the vt query parameter of the referer unit
        //  -> will be used when coming back to the REFERER unit
        return `<li>
            <h${level} class="related-entry related-unit">
                <a class="nav-element ${current ? "current" : ""}" 
                  href="${ current ? '#' : (url + "?ref="+referer + (paramVT || refererParamVT ? "?vt=" + (paramVT || refererParamVT): "") + originQueryParam + refererVTQueryParam) }"
...
(function () {
    ...
    const generateSelectedQuestions = function ($categories, num) {
        ...
        return $categories.toArray().flatMap((cat, cIdx) => 
            $(cat).find('.roulette-question').toArray().map((q, qIdx) => [cIdx, qIdx]))
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $(".widget-roulette").each(function() {
            ...
            const $widget = $(this);
            const $container = $widget.closest('.content-element');
            const $elementsToHideWhenQuestion = $container.find('.container-roulette,.roulette-lifes,.roulette-heading');
            const id = $widget.attr('id');
            const content = JSON.parse(b64DecodeUnicode(this.dataset.content));
            const $circle = $widget.find(".roulette-circle");
            const $categories = $widget.find(".roulette-category");
            ...
            let selectedQuestions = generateSelectedQuestions($categories, content.lifes);
            ...
            setTimeout(() => $widget.trigger('gamification-start-teacher', [initialQuestions]), 1000);
            ...
            let initTimer = function() {
                ...
                let updateTimer = () => {
                    ...
                    if (current > time) {
                        ...
                        $widget.find(".roulette-question").toggleClass("show", false);
                        $elementsToHideWhenQuestion.removeClass("d-none");
                        $widget.find('.roulette-info').text(info);
                        $widget.addClass("finished");
                        $widget.find('.roulette-spin').focus().prop('disabled', true);
                        $widget.find('.btn-check-answer').prop('disabled', true);
                        $container.find('.speech-to-action.active').last().trigger('speech-hidden');
                        return;
                    }
            ...
            $widget.on("click", ".roulette-spin", function () {
                ...
                if (!selectedQuestions.length) {
                    ...
                    let info = i18n("roulette-finished");
                    // This is the last question: notify gamification results
                    $widget.trigger('gamificate', [totalCorrect == content.lifes, totalScore/content.lifes, id]);
                    if (!gamification) {
                        info += " - " + i18n("TextNumberCorrect", {num: totalCorrect});
                ...
                $currentQuestion = $categories.eq(selectedCategory).find('.roulette-question').eq(selectedQuestion);
                ...
                $circle[0].offsetHeight; /* trigger reflow again */
                // When the animation finishes...
                $circle.one("animationend", function () {
                    $spinner.prop('disabled', false);
                    $currentQuestion.addClass('show').focus();
                    $circle[0].style.setProperty('--final', "0deg");
                    $widget.find('.roulette-info').text(i18n("roulette-category", {category: $categories.eq(selectedCategory)[0].dataset.name}));
                    $elementsToHideWhenQuestion.addClass("d-none");
                    $container.find('.speech-to-action.active').last().trigger('speech-hidden');
                })
            });

            // Reset the whole activity
            $widget.on('click', '.btn-roulette-reset', function (e) {
                answers = {};
                $widget.find('.btn-check-answer').prop('disabled', false);
                $widget.find(".roulette-question").toggleClass("show", false);
                $widget.find(".btn-reset").trigger('click');
                ...
                $widget.removeClass('finished');
                $widget.find('.roulette-spin').prop('disabled', false).focus();
                $lifes.toggleClass("fa-regular", false).toggleClass("fa-solid", true).addClass('d-none');
                $lifesContainer.attr('aria-label', i18n('roulette-lifes', {num: content.lifes}));
                $circle[0].style.setProperty('--final', "0deg");
                selectedQuestions = generateSelectedQuestions($categories, content.lifes);
                $lifes.each(function (index) { if (index < selectedQuestions.length) $(this).removeClass('d-none') });
                // Reset the total counts
                totalScore = 0;
                totalCorrect = 0;
            ...
            $widget.on('gamificate', function (e, correct, score, id) {
                ...
                // Only propagate gamification events from the current widget
                // Children events will be handled by this widget
                e.stopPropagation();
                answers[id] = [correct, score];
                // Get the sum of score in the answers object
                totalCorrect = Object.values(answers).map(arr => arr[0]).reduce((acc, correct) => acc + (correct ? 1 : 0), 0);
                totalScore = Object.values(answers).map(arr => arr[1]).reduce((acc, score) => acc + score, 0);
                if (gamification) $(e.target).attr("disabled", true);
                if (correct || gamification)  {
                    const timeoutMS = gamification ? 0 : 3000;
                    setTimeout(() => {
                        $elementsToHideWhenQuestion.removeClass("d-none");
                        $widget.find('.roulette-spin').focus();
                        $widget.find(".roulette-question.show").toggleClass("show", false);
                        $container.find('.speech-to-action.active').last().trigger('speech-hidden');
                    }, timeoutMS);
                }
            });

            $widget.on('gamification-start-student', function (e, questions) {
            ...
            let onSpeechToAction = function (event, action, entities) {
                ...
                const $spin = $widget.find('.roulette-spin');
                if (action === "FINISH") {
                    !$spin.is(':visible') && event.preventDefault();
                    return;
                }
                ...
                if (action === "START" || action === "CURRENT" || action === "NEXT" || 
                    ...
                    switch (action) {
                        case "SELECT": $spin.trigger('click'); break;
                        case "RESET": $widget.find('.btn-roulette-reset').trigger('click'); break;
                        case "REPEAT": $(self).find('.tta-message').append(makeAlert('alert-info', i18n('tta-info-containertest'))); break;
                        case "CHECK":
                        case "NONE":
                        default:
...
(function () {
    ...
    let onLanguageLoaded = function (err, i18n) {
        ...
        $('.test-question').each(function () {
            ...
            const gamification = $('body').hasClass('gamification') && !$('body').hasClass('gamification-teacher');
            const $widget = $(this);
            let $question = $(this).find('.question').first();
            let self = this;
            let id = $(self).attr('id');
            let $options = $(self).find('input');
            ...
            let onSpeechToAction = function (event, action, entities) {
                ...
                let $checkAnswerBtn = $(self).find('.btn-check-answer')
                let $selected = $options.filter(function () { return $(this).is(':focus') }).first()
                if (!$selected.length) {
                ...
                if (action === "START" || action === "CURRENT")
                    ...
                    let position = $options.index($selected);
                    let nextPos = action === "NEXT" ? ((position + 1) % $options.length) : ((position - 1) % $options.length);
                    $options.eq(nextPos).focus();
                }
                else if (action === "GOTO") {
                    let nextPos = (parseInt(entities['target-position']) - 1) % $options.length;
                    $options.eq(nextPos).focus();
                }
                else if (action === "CLOSE" && ($(".modal.show").length || $(".alert").length)) {
                    $(".modal.show").length && $('.modal.show').modal('hide');
            ...
            $widget.on('click', '.btn-check-answer', function (e) {
                ...
                if (!gamification) {
                    ...
                    $question.find('.negativeFeedback:not(:empty)').toggleClass('hidden', $checkedAnswers.length == 0 || correct);
                    $question.find('.positiveFeedback:not(:empty)').toggleClass('hidden', $checkedAnswers.length == 0 || !correct);
                    // Show the user immediate global feedback
                    $widget.find('.resultado').html(i18n("TextNumberCorrect", { num: $correctAnswers.length }));
                    if (correct) setObjetivoCompleto(`objetivo${id}`, "«widget.name»", "«widget.name»");
                }
            ...
            $widget.find('input').on('change', function (e) {
